...
def total_count_by_hour(hour_df):
...
main_df_days = days_df[
    (days_df["dteday"] >= str(start_date)) & (days_df["dteday"] <= str(end_date))
]

main_df_hour = hours_df[
    (hours_df["dteday"] >= str(start_date)) & (hours_df["dteday"] <= str(end_date))
]

hour_count_df = total_count_by_hour(main_df_hour)
...
st.subheader("In what season is bike rental peak?")

colors = ["#D3D3D3", "#D3D3D3", "#D3D3D3", "#ff9999"]
...
ax.set_title("Season Graph", loc="center", fontsize=50)
ax.set_ylabel(None)
ax.set_xlabel(None)
ax.tick_params(axis="x", labelsize=35)
ax.tick_params(axis="y", labelsize=30)
st.pyplot(fig)
st.subheader("Hours with lots and few of bike renters?")
...
sns.barplot(
    ...
    palette=["#D3D3D3", "#D3D3D3", "#ff9999", "#D3D3D3", "#D3D3D3"],
    ax=ax[0],
)
ax[0].set_ylabel(None)
ax[0].set_xlabel("Hours (PM)", fontsize=30)
ax[0].set_title("Hour with lot of renters", loc="center", fontsize=30)
...
sns.barplot(
    ...
    palette=["#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3", "#ff9999"],
    ax=ax[1],
)
ax[1].set_ylabel(None)
ax[1].set_xlabel("Hours (AM)", fontsize=30)
ax[1].set_title("Hour with a few of renters", loc="center", fontsize=30)
ax[1].invert_xaxis()
...
st.subheader("How has the company's sales performance been in recent years?")

fig, ax = plt.subplots(figsize=(16, 8))
ax.plot(
    days_df["dteday"], days_df["count_cr"], marker="o", linewidth=2, color="#90CAF9"
)
